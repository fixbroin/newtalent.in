...
const clearLocalStorageItems = async (uid?: string) => {
    ...
    if (typeof window !== 'undefined') {
        localStorage.removeItem('newtalentScheduledDate');
        localStorage.removeItem('newtalentScheduledTimeSlot');
        localStorage.removeItem('newtalentEstimatedEndTime');
        localStorage.removeItem('newtalentCustomerAddress');
        ...
        localStorage.removeItem('newtalentAppliedPromoCode');
        ...
        localStorage.removeItem('newtalentAppliedPlatformFees');
        ...
        localStorage.removeItem('newtalentPaymentMethod');
        localStorage.removeItem('newtalentFinalBookingTotal');
...
export default function ThankYouPage() {
  ...
  const [isSubscriptionConfirmation, setIsSubscriptionConfirmation] = useState(false);
  const [subscriptionPlanDetails, setSubscriptionPlanDetails] = useState<SubscriptionPlan | null>(null);
  ...
  useEffect(() => {
    ...
    const processPage = async () => {
      ...
      const paymentMethod = localStorage.getItem('newtalentPaymentMethod');
      ...
      // --- 2. Handle Cancellation Fee Payment Verification ---
      ...
      if (cartEntriesFromStorage.length === 0) {
        toast({ title: "Booking Processed", description: "Redirecting to My Profile.", variant: "default" });
        router.push('/profile');
        setIsLoadingPage(false);
        return;
      }

      // --- 3. Handle Regular Booking Confirmation ---
      if (isOnlinePayment) {
      ...
      try {
        ...
        if (typeof window !== 'undefined') {
          customerEmail = localStorage.getItem('newtalentCustomerEmail') || customerEmail;
          scheduledDateStored = localStorage.getItem('newtalentScheduledDate') || scheduledDateStored; 
          scheduledTimeSlot = localStorage.getItem('newtalentScheduledTimeSlot') || scheduledTimeSlot;
          estimatedEndTime = localStorage.getItem('newtalentEstimatedEndTime') || undefined;
          bookingDiscountCode = localStorage.getItem('newtalentBookingDiscountCode') || undefined;
          const discountAmountStr = localStorage.getItem('newtalentBookingDiscountAmount');
          bookingDiscountAmount = discountAmountStr ? parseFloat(discountAmountStr) : undefined;
          appliedPromoCodeId = localStorage.getItem('newtalentAppliedPromoCodeId') || undefined;
          const platformFeesStr = localStorage.getItem('newtalentAppliedPlatformFees');
          ...
          const addressDataString = localStorage.getItem('newtalentCustomerAddress');
        ...
        try {
           ...
           if (currentUser?.uid) {
              const userNotification: Omit<FirestoreNotification, 'id'> = {
                ...
                message: `Your booking ${newBookingId} has been successfully placed. We'll assign a Artist shortly.`,
                type: 'success',
                href: '/my-bookings',
                read: false,
                createdAt: Timestamp.now(),
  ...
  if (isLoadingPage || !isMounted || isLoadingAppSettings || (!bookingDetailsForDisplay && !isCancellationConfirmation && !isSubscriptionConfirmation)) {
    return (
      ...
      <div className="max-w-2xl mx-auto px-2 sm:px-0">
        <Card className="shadow-lg text-center">
          <CardHeader className="items-center px-4 sm:px-6">
            <Sparkles className="h-12 w-12 sm:h-16 sm:w-16 text-primary animate-pulse mb-4" />
            <CardTitle className="text-2xl sm:text-3xl font-headline">Subscription Activated!</CardTitle>
            <CardDescription className="text-md sm:text-lg text-muted-foreground">
                You have successfully subscribed to the <strong>{subscriptionPlanDetails?.name}</strong> plan.
            </CardDescription>
          </CardHeader>
          <CardContent className="px-4 sm:px-6 md:px-8 text-sm">
             <div className="p-4 bg-secondary/30 rounded-2xl border mb-4">
                <p>Plan: <strong>{subscriptionPlanDetails?.name}</strong></p>
                <p>Duration: <strong>{subscriptionPlanDetails?.durationDays} Days</strong></p>
                <p>Status: <span className="text-green-600 font-bold">Active</span></p>
             </div>
             <p className="text-muted-foreground">You can now connect with artists and request services across the platform.</p>
          </CardContent>
          <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pt-4 sm:pt-6">
            ...
            <Link href="/profile" passHref>
              <Button size="lg" className="w-full sm:w-auto text-sm sm:text-base"><UserCircle className="mr-2 h-4 w-4" /> Go to My Profile</Button>
            </Link>
          </CardFooter>
        </Card>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-2 sm:px-0">
        <Card className="shadow-lg text-center">
          ...
          <CardFooter className="flex flex-col sm:flex-row gap-3 sm:gap-4 justify-center pt-4 sm:pt-6">
            ...
            <Link href="/profile" passHref><Button size="lg" className="w-full sm:w-auto text-sm sm:text-base"><UserCircle className="mr-2 h-4 w-4" /> Go to My Profile</Button></Link>
          </CardFooter>
        </Card>
      </div>
    );
  ...
  <Link href="/profile" passHref className="w-full sm:w-auto">
    ...
    >
      <UserCircle className="mr-2 h-4 w-4" /> Go to My Profile
    </Button>
  </Link>
</CardFooter>
      </Card>
