...
import type { FirestoreService, FirestoreUser, FirestorePromoCode, AppSettings, PlatformFeeSetting, AppliedPlatformFeeItem, PriceVariant, SubscriptionPlan } from '@/types/firestore';
...
export default function PaymentPage() {
  ...
  const canOfferPayLater = useMemo(() => {
    ...
    if (isCancellationFeeMode || isSubscriptionMode) return false; 
  ...
  }, [appConfig.enableCOD, cartEntries, serviceDetailsMap, isCancellationFeeMode, isSubscriptionMode]);
  ...
  const loadInitialData = useCallback(async () => {
    if (isSubscriptionMode || isCancellationFeeMode) return;
    ...
    if (currentCartEntries.length === 0 && !isSubscriptionMode && !isCancellationFeeMode) {
    ...
    try {
      ...
      const storedPromo = localStorage.getItem('newtalentAppliedPromoCode');
      if (storedPromo) { try { const parsedPromo: AppliedPromoCodeInfo = JSON.parse(storedPromo); setAppliedPromoCode(parsedPromo); setPromoCodeInput(parsedPromo.code); } catch (e) { localStorage.removeItem('newtalentAppliedPromoCode'); } }
  ...
  }, [pathname, currentUser, toast, isSubscriptionMode, isCancellationFeeMode]);
  ...
  useEffect(() => {
    ...
    if (reason === 'cancellation_fee') {
      ...
      if (planId) {
        ...
        setIsLoadingCartDetails(true);
        getDoc(doc(db, 'adminSubscriptionPlans', planId)).then(snap => {
          if (snap.exists()) {
            const plan = { id: snap.id, ...snap.data() } as SubscriptionPlan;
            setSubscriptionPlan(plan);
            setTotalAmountDue(plan.price);
            setSumOfDisplayedItemPrices(plan.price);
  ...
  useEffect(() => {
    if (!isMounted || isLoadingAppSettings || isCancellationFeeMode || isSubscriptionMode) return;

    loadInitialData();
    
    // Listen for cart changes from other tabs/windows
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === 'newtalentUserCart') {
        loadInitialData();
      }
    };
  ...
  useEffect(() => {
    if (isLoadingCartDetails || isLoadingAppSettings || isCancellationFeeMode || isSubscriptionMode || cartEntries.length === 0) {
      if (cartEntries.length === 0 && !isLoadingCartDetails && !isLoadingAppSettings && !isCancellationFeeMode && !isSubscriptionMode) {
  ...
  const handleApplyPromoCode = async () => {
    ...
    try {
      ...
      setAppliedPromoCode(appliedInfo); localStorage.setItem('newtalentAppliedPromoCode', JSON.stringify(appliedInfo));
  ...
  const handleRemovePromoCode = (silent = false) => { setAppliedPromoCode(null); setPromoCodeInput(""); localStorage.removeItem('newtalentAppliedPromoCode'); if(!silent) toast({ title: "Promo Removed", description: "Discount has been removed." }); };
  ...
  const handlePaymentAction = async () => {
    ...
    if (paymentMethod === 'later' && !isCancellationFeeMode) {
        localStorage.setItem('newtalentPaymentMethod', 'Pay After Service');
        localStorage.setItem('newtalentFinalBookingTotal', totalAmountDue.toString());
        if (appliedPromoCode) {
            localStorage.setItem('newtalentBookingDiscountCode', appliedPromoCode.code);
            localStorage.setItem('newtalentBookingDiscountAmount', appliedPromoCode.calculatedDiscount.toString());
            localStorage.setItem('newtalentAppliedPromoCodeId', appliedPromoCode.id);
        } else {
            localStorage.removeItem('newtalentBookingDiscountCode');
            localStorage.removeItem('newtalentBookingDiscountAmount');
            localStorage.removeItem('newtalentAppliedPromoCodeId');
        }
        if (calculatedPlatformFees.length > 0) localStorage.setItem('newtalentAppliedPlatformFees', JSON.stringify(calculatedPlatformFees)); else localStorage.removeItem('newtalentAppliedPlatformFees');
    ...
    try {
      ...
      const customerAddressDataString = localStorage.getItem('newtalentCustomerAddress');
      ...
      const paymentDescription = isSubscriptionMode && subscriptionPlan 
        ? `Subscription: ${subscriptionPlan.name}`
        : (isCancellationFeeMode && cancellationFeeDetails?.humanReadableBookingId ? `Cancellation Fee for Booking ${cancellationFeeDetails.humanReadableBookingId}` : "Service Booking Payment");

      const options = {
        key: appConfig.razorpayKeyId, amount: orderDetails.amount, currency: "INR", name: globalSettings?.websiteName || "Newtalent Services",
        ...
        handler: (response: any) => {
          ...
          localStorage.setItem('newtalentPaymentMethod', 'Online'); // Set generic Online for successful razorpay
          localStorage.setItem('newtalentFinalBookingTotal', totalAmountDue.toString());

          if (isSubscriptionMode && subscriptionPlan) {
            localStorage.setItem('newtalentPendingSubscriptionPlanId', subscriptionPlan.id);
          }
  ...
  const breadcrumbItems: BreadcrumbItem[] = [
    ...
    { label: isCancellationFeeMode ? "Pay Cancellation Fee" : isSubscriptionMode ? "Checkout Subscription" : "Payment Details" },
  ];
    ...
    <div className="max-w-2xl mx-auto">
      ...
      <Card className="shadow-lg">
        ...
        <CardContent className="space-y-6">
          ...
          {(cartEntries.length > 0 || isCancellationFeeMode || isSubscriptionMode) && (
            <>
              {!isCancellationFeeMode && !isSubscriptionMode && (<>
              ...
              {isSubscriptionMode && subscriptionPlan && (
                <div className="p-4 bg-primary/5 border border-primary/20 rounded-2xl mb-4">
                  <h3 className="font-bold text-lg mb-1">Subscription Summary</h3>
                  <p className="text-muted-foreground text-sm">Plan: <strong>{subscriptionPlan.name}</strong></p>
                  <p className="text-muted-foreground text-sm">Duration: <strong>{subscriptionPlan.durationDays} Days</strong></p>
                </div>
              )}
              ...
              <div className="border-t pt-4 space-y-2 mt-4">
                {!isCancellationFeeMode && !isSubscriptionMode ? (
                    ...
                    subscriptionPlan && (
                        <div className="flex justify-between font-semibold text-md">
                            <span>Subscription Price ({subscriptionPlan.name}):</span>
                            <span>₹{subscriptionPlan.price.toFixed(2)}</span>
                        </div>
                    )
                )}
        ...
        <CardFooter className="flex flex-col sm:flex-row justify-between gap-2 mt-4">
          ...
          <Button
            ...
            disabled={ (isCancellationFeeMode ? !cancellationFeeDetails : isSubscriptionMode ? !subscriptionPlan : cartEntries.length === 0) || isLoadingCartDetails || isLoadingAppSettings || currentAvailablePaymentOptions.length === 0 || (!paymentMethod && !isCancellationFeeMode) || isProcessingPayment}
            className="w-full sm:w-auto"
          >
            {isProcessingPayment ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
            {isProcessingPayment ? 'Processing...' : (isCancellationFeeMode ? 'Pay Cancellation Fee' : isSubscriptionMode ? 'Activate Subscription' : (paymentMethod === 'later' ? 'Confirm Booking' : 'Confirm Booking & Pay'))}
