...
export default function AdminActivityFeedPage() {
  ...
  const renderUserCell = (activity: GroupedUserActivity) => {
    ...
    const name = activity.userId && userNames[activity.userId]
      ? userNames[activity.userId]
      : (activity.userDisplayName || (isGuest ? 'Guest User' : 'Registered User'));
  ...
  const renderMobileCard = (activity: GroupedUserActivity, idx: number) => {
    ...
    const name = activity.userId && userNames[activity.userId]
      ? userNames[activity.userId]
      : (activity.userDisplayName || (isGuest ? 'Registered User' : 'Guest User'));
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-3 bg-muted/20 p-3 rounded-2xl border border-dashed border-muted-foreground/20">
            <Avatar className="h-10 w-10 border shadow-sm shrink-0">
              <AvatarFallback className={cn("text-xs font-black uppercase", isGuest ? "bg-slate-100 text-slate-500" : "bg-primary/10 text-primary")}>
                {isGuest ? <User className="h-4 w-4" /> : name.charAt(0)}
              </AvatarFallback>
            </Avatar>
            <div className="min-w-0">
