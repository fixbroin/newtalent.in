...
export default function AdminBlogPage() {
  ...
  const handleDeletePost = async (post: FirestoreBlogPost) => {
    ...
    try {
      ...
      if (post.slug && post.isPublished) {
        submitPathToGoogleIndexing(`/blog/${post.slug}`, 'URL_DELETED').catch(err => {
  ...
  const handleTogglePublished = async (post: FirestoreBlogPost) => {
    ...
    try {
      ...
      await triggerRefresh('sitemap');
      if (post.slug) {
        submitPathToGoogleIndexing(`/blog/${post.slug}`, !post.isPublished ? 'URL_UPDATED' : 'URL_DELETED').catch(err => {
    ...
    <div className="space-y-6">
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => { if (!isSubmitting) { setIsFormOpen(open); if (!open) setEditingPost(null); } }}>
        <DialogContent className="w-[90vw] max-w-4xl max-h-[90vh] p-0 flex flex-col">
          <DialogHeader className="p-2 pb-4 border-b"><DialogTitle>{editingPost ? 'Edit Blog Post' : 'Create New Blog Post'}</DialogTitle></DialogHeader>
          <BlogForm 
