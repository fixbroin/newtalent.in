...
import type { WithdrawalRequest, WithdrawalStatus, FirestoreNotification, FirestoreUser, ArtistApplication } from '@/types/firestore';
...
export default function ArtistWithdrawalsPage() {
  ...
  const [Artists, setArtists] = useState<FirestoreUser[]>([]);
  ...
  const loadArtists = async () => {
    ...
    try {
        // 1. Fetch only approved Artist applications (Very efficient)
        const appsQuery = query(collection(db, "ArtistApplications"), where("status", "==", "approved"));
        const appsSnapshot = await getDocs(appsQuery);
        const approvedApps = appsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as ArtistApplication));
        ...
        setArtists(ArtistList.sort((a, b) => (b.withdrawableBalance || 0) - (a.withdrawableBalance || 0)));
    } catch (error) {
        console.error("Error loading Artists:", error);
        toast({ title: "Error", description: "Could not load Artist balances.", variant: "destructive" });
    } finally {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      setRequests(allRequests.filter(req => req.artistId !== 'referral_system'));
      setIsLoading(false);
    }, (error) => {
  ...
  const handleUpdateStatus = async (request: WithdrawalRequest, newStatus: WithdrawalStatus, reason?: string) => {
    ...
    const userDocRef = doc(db, "users", request.artistId);
    const requestDocRef = doc(db, "withdrawalRequests", request.id);

    try {
      await runTransaction(db, async (transaction) => {
        const userDoc = await transaction.get(userDocRef);
        if (!userDoc.exists()) throw new Error("Artist user not found.");
        ...
        if (newStatus === 'rejected' || newStatus === 're_submit') {
          ...
          // Actually, let's look at how we want to handle the Artist's balance.
          ...
          notificationType = newStatus === 'rejected' ? 'error' : 'warning';
            ...
            // Note: Withdrawals are already added to stats when REQUESTED by Artist.
      ...
      // Trigger actual Push Notification for the Artist
  ...
  const handleDeleteRequest = async (request: WithdrawalRequest) => {
    ...
    try {
        await runTransaction(db, async (transaction) => {
            ...
            // the money was deducted but NOT yet finalized. Deleting it should REFUND the Artist.
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
            ...
            <p className="text-muted-foreground text-sm mt-1">Manage payout requests and track Artist earnings.</p>
        </div>
      </div>
      ...
      <Tabs defaultValue="requests" className="w-full">
        <TabsList className="grid w-full grid-cols-2 max-w-md mb-6 h-12 p-1 bg-muted/50 rounded-xl">
          ...
          <TabsTrigger value="balances" onClick={loadArtists} className="rounded-lg font-bold">
            <Users className="mr-2 h-4 w-4"/> Artist Balances
          </TabsTrigger>
        </TabsList>
        ...
        <TabsContent value="requests">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Process pending payout requests from service Artists.</CardDescription>
            </CardHeader>
            <CardContent>
              {requests.length === 0 ? (
                ...
                <Table>
                  <TableHeader><TableRow><TableHead>Artist</TableHead><TableHead>Amount</TableHead><TableHead>Method</TableHead><TableHead>Details</TableHead><TableHead>Requested</TableHead><TableHead>Status</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {requests.map(req => {
                       const ArtistProfile = Artists.find(p => p.uid === req.artistId);
                       const currentBalance = ArtistProfile?.withdrawableBalance ?? null;

                       return (
                       <TableRow key={req.id}>
                          <TableCell>
                            <div className="font-medium">{req.artistName}</div>
                            <div className="text-xs text-muted-foreground">{req.artistEmail}</div>
                            {currentBalance !== null && (
                                  ...
                                  <AlertDialog>
                                      ...
                                      <AlertDialogContent>
                                          <AlertDialogHeader>
                                              ...
                                              <AlertDialogDescriptionComponent>Permanently remove this request record. This action will adjust the Artist's balance if the request was not yet completed.</AlertDialogDescriptionComponent>
                                          </AlertDialogHeader>
                                          <AlertDialogFooter>
        ...
        <TabsContent value="balances">
          <Card>
            <CardHeader>
              <div className="flex justify-between items-center">
                <div>
                    <CardTitle>Artist Earnings Overview</CardTitle>
                    <CardDescription>Current withdrawable balances and lifetime payouts for all Artists.</CardDescription>
                </div>
                <Button variant="outline" size="sm" onClick={loadArtists} disabled={isLoadingArtists}>
                    {isLoadingArtists ? <Loader2 className="h-4 w-4 animate-spin mr-2"/> : <RefreshCw className="h-4 w-4 mr-2"/>} Refresh
                </Button>
              </div>
            </CardHeader>
            <CardContent>
              {isLoadingArtists ? (
                ...
                <p className="text-center py-10 text-muted-foreground">No Artists found.</p>
              ) : (
                <Table>
                  <TableHeader><TableRow><TableHead>Artist</TableHead><TableHead>Month Gross</TableHead><TableHead>Month Net</TableHead><TableHead>Wallet Balance</TableHead><TableHead>Lifetime Paid</TableHead><TableHead>Status</TableHead></TableRow></TableHeader>
                  <TableBody>
                    {Artists.map(p => {
                       const now = new Date();
                       const monthKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
      ...
      <AlertDialog open={showRejectionDialog} onOpenChange={setShowRejectionDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescriptionComponent>Please provide a reason. This will be sent to the Artist and the amount will be refunded to their wallet.</AlertDialogDescriptionComponent>
          </AlertDialogHeader>
        ...
        <DialogContent className="sm:max-w-md">
            <DialogHeader>
                ...
                <DialogDescriptionComponent>
                    Request from: <span className="font-semibold">{selectedRequestForDetails?.artistName}</span>
                </DialogDescriptionComponent>
            </DialogHeader>
