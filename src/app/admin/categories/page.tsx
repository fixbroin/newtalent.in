...
export default function AdminCategoriesPage() {
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === 'newtalentCategoryNameOverrides') {
  ...
  const handleDeleteCategory = async (categoryId: string) => {
    ...
    try {
      ...
      if (catToDelete && catToDelete.slug) {
        submitPathToGoogleIndexing(`/category/${catToDelete.slug}`, 'URL_DELETED').catch(err => {
          console.error("Google Indexing error on category delete:", err);
        });
      }
    } catch (error) {
  ...
  const handleToggleActive = async (category: FirestoreCategory) => {
    ...
    try {
        ...
        await triggerRefresh('global-cache');
        if (category.slug) {
          submitPathToGoogleIndexing(`/category/${category.slug}`, !category.isActive ? 'URL_UPDATED' : 'URL_DELETED').catch(err => {
            console.error("Google Indexing error on category toggle active:", err);
          });
        }
    } catch (error) {
  ...
  const handleFormSubmit = async (data: Omit<FirestoreCategory, 'id' | 'createdAt'> & { id?: string, slug?: string }) => {
    ...
    const payloadForFirestore: Partial<Omit<FirestoreCategory, 'id' | 'createdAt'>> = {
      ...
      isPortfolioRequired: data.isPortfolioRequired ?? true,
    ...
    try {
      ...
      const categorySlug = payloadForFirestore.slug || data.slug || generateSlug(data.name);
      if (categorySlug) {
        submitPathToGoogleIndexing(`/category/${categorySlug}`, 'URL_UPDATED').catch(err => {
          console.error("Google Indexing error on category save:", err);
        });
      }
  ...
  return (
    <div className="space-y-6">
      ...
      <Dialog open={isFormOpen} onOpenChange={(open) => { if (!isSubmitting) { setIsFormOpen(open); if (!open) setEditingCategory(null); } }}>
        <DialogContent className="w-full max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl max-h-[90vh] overflow-y-auto p-2">
          <DialogHeader>
