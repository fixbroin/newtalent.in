...
export async function POST(request: Request) {
  try {
    ...
    // --- NEW: Update Artist's withrawableBalance and System Stats on Completion ---
    const isCashPayment = (method: string) => method === 'Pay After Service' || method === 'Cash on Delivery';
    if (isCompleted && booking.ArtistId) {
        const calculateArtistFee = (bookingAmount: number, feeType?: string, feeValue?: number): number => {
        ...
        const commission = calculateArtistFee(booking.totalAmount, appConfig.ArtistFeeType, appConfig.ArtistFeeValue);
        ...
        const ArtistDocRef = adminDb.collection('users').doc(booking.ArtistId);
        tasks.push(adminDb.runTransaction(async (transaction) => {
            ...
            const currentWithdrawableBalance = ArtistData?.withdrawableBalance || 0;
            const commission = calculateArtistFee(booking.totalAmount, appConfig.ArtistFeeType, appConfig.ArtistFeeValue);
            
            // Monthly Stats Logic
            const now = new Date();
            const monthKey = `${now.getFullYear()}-${String(now.getMonth() + 1).padStart(2, '0')}`;
            let stats = ArtistData?.monthlyStats || { monthKey, gross: 0, commission: 0, cashCollected: 0, withdrawals: 0, onlineNet: 0, cashCommission: 0 };
    ...
    const emailFlowInput = {
        ...
        siteName: seoSettings?.websiteName || "Newtalent",
