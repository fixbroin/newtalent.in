...
export default function ArtistNotificationsPage() {
  const { user: ArtistUser, isLoading: authLoading } = useAuth();
  ...
  useEffect(() => {
    if (!isMounted || !ArtistUser || authLoading) {
      if (!authLoading && !ArtistUser && isMounted) setIsLoadingNotifications(false);
      return;
    }
    ...
    return () => unsubscribe();
  }, [ArtistUser, authLoading, toast, isMounted]);

  const handleMarkAsRead = async (notificationId?: string) => {
    if (!ArtistUser || !notificationId) return;
  ...
  const handleMarkAllAsRead = async () => {
    if (!ArtistUser || notifications.filter(n => !n.read).length === 0) return;
  ...
  const handleClearAllArtistNotifications = async () => {
    if (!ArtistUser) return;
    setIsClearing(true);
    try {
      const notificationsCollectionRef = collection(db, "userNotifications");
      const q = query(notificationsCollectionRef, where("userId", "==", ArtistUser.uid));
      ...
      console.error("Error clearing Artist notifications: ", error);
  ...
  if (!ArtistUser && isMounted) {
    return (
      <div className="container mx-auto px-4 py-8 max-w-2xl text-center">
        ...
        <Link href="/auth/login?redirect=/artist/notifications" passHref>
          <Button>Login</Button>
        </Link>
      </div>
    );
              ...
              <AlertDialog>
                ...
                <AlertDialogContent>
                  ...
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction onClick={handleClearAllArtistNotifications} disabled={isClearing} className="bg-destructive hover:bg-destructive/90">
