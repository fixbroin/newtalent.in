...
export default function ArtistMyReviewsPage() {
  const { user: ArtistUser, isLoading: authIsLoading } = useAuth();
  ...
  useEffect(() => {
    if (!ArtistUser || authIsLoading) {
      if (!authIsLoading && !ArtistUser) setIsLoading(false);
      return;
    }
    setIsLoading(true);
    
    // 1. Fetch all bookings assigned to this Artist (Limit to 50 most recent)
    const bookingsQuery = query(
      collectionGroup(db, "bookings"), 
      where("ArtistId", "==", ArtistUser.uid),
    ...
    const unsubscribeBookings = onSnapshot(bookingsQuery, async (bookingsSnapshot) => {
      ...
      const ArtistBookingIds = bookingsSnapshot.docs.map(doc => doc.data().bookingId as string);

      if (ArtistBookingIds.length === 0) {
        setReviews([]);
        setIsLoading(false);
        return;
      }
      ...
      // If ArtistBookingIds can exceed 30, this part needs to be refactored to batch the 'in' query.
      const CHUNK_SIZE = 30;
      const reviewPromises = [];

      for (let i = 0; i < ArtistBookingIds.length; i += CHUNK_SIZE) {
        const chunk = ArtistBookingIds.slice(i, i + CHUNK_SIZE);
        if (chunk.length > 0) {
      ...
      console.error("Error fetching Artist bookings for reviews:", error);
      toast({ title: "Error", description: "Could not fetch data for reviews.", variant: "destructive" });
      setIsLoading(false);
    });

    return () => unsubscribeBookings();
  }, [ArtistUser, authIsLoading, toast]);
