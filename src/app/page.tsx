...
export async function generateMetadata(
  ...
  const title = seoSettings.homepageMetaTitle || seoSettings.siteName || 'Newtalent';
  ...
  const siteName = resolvedParent.openGraph?.siteName || seoSettings.siteName || 'Newtalent';
...
export default async function Page() {
  ...
  const appBaseUrl = getBaseUrl();
  const siteName = homepageData.seoSettings.siteName || 'Newtalent';
  ...
  const localBusinessSchema = {
    ...
    "@type": seoSettings.structuredDataType || "EmploymentAgency",
    "additionalType": "https://schema.org/EntertainmentBusiness",
    "name": siteName,
    "url": appBaseUrl,
    "logo": `${appBaseUrl}/android-chrome-512x512.png`,
    "image": schemaImage,
    "description": seoSettings.homepageMetaDescription || "India's number one platform for acting jobs, film auditions, and casting calls. Discover top actors in Bangalore.",
