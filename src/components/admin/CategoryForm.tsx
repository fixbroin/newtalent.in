...
const categoryFormSchema = z.object({
  ...
  isPortfolioRequired: z.boolean().default(true), // Added
...
export default function CategoryForm({ onSubmit: onSubmitProp, initialData, onCancel, isSubmitting: isParentSubmitting = false }: CategoryFormProps) {
  ...
  const form = useForm<CategoryFormData>({
    ...
    defaultValues: {
      name: "", slug: "", order: 0, isActive: true, isPortfolioRequired: true, imageUrl: "", imageHint: "",
  ...
  useEffect(() => {
    if (initialData) {
      form.reset({
        ...
        isPortfolioRequired: initialData.isPortfolioRequired === undefined ? true : initialData.isPortfolioRequired,
  ...
  const handleSubmit = async (formData: CategoryFormData) => {
    ...
    try {
      ...
      await onSubmitProp({
        ...
        isPortfolioRequired: formData.isPortfolioRequired,
  ...
  return (
    <Form {...form} key={initialData ? `edit-${initialData.id}` : 'new-post'}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="flex-grow space-y-6 p-2 overflow-y-auto">
        <FormField
        ...
        <FormField
          control={form.control}
          name="isPortfolioRequired"
          render={({ field }) => (
            <FormItem className="flex flex-row items-center justify-between rounded-lg border p-3 shadow-sm bg-background/50">
              <div className="space-y-0.5">
                <FormLabel>Portfolio Photos Required</FormLabel>
                <FormDescription>If enabled, artists in this category must upload portfolio photos (Step 3).</FormDescription>
              </div>
              <FormControl>
                <Switch checked={field.value} onCheckedChange={field.onChange} disabled={effectiveIsSubmitting}/>
              </FormControl>
        ...
        <div className="space-y-4 pt-4 border-t">
          ...
          <FormField control={form.control} name="seo_title" render={({ field }) => (
            <FormItem><FormLabel>Meta Title</FormLabel><FormControl><Input placeholder="e.g., Home Repair Services | Newtalent" {...field} disabled={effectiveIsSubmitting} /></FormControl><FormMessage /></FormItem>
          )}/>
