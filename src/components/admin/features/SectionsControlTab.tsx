...
import { Loader2, Save, LayoutGrid, Star, Clock, ListChecks, FileText, Construction, Activity, Sparkles } from "lucide-react"; 
...
const sectionsControlSchema = z.object({
  ...
  isSubscriptionRequired: z.boolean().default(true),
});
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        form.reset({
          ...
          isSubscriptionRequired: data.isSubscriptionRequired ?? defaultFeaturesConfig.isSubscriptionRequired,
        });
      } else {
        ...
        <Card>
          ...
          <CardContent className="space-y-6 p-2">
                ...
                render={({ field }) => (
                  <FormItem className="flex flex-row items-center justify-between rounded-lg border bg-background p-4 shadow-sm">
                    <div className="space-y-0.5">
                      <FormLabel className="text-base flex items-center">Require Subscription</FormLabel>
                      <FormDescription>If enabled, users must have a paid plan to send requests or register as an artist.</FormDescription>
                    </div>
            ...
            <div className="space-y-4">
               <h3 className="text-sm font-bold flex items-center mb-3 text-muted-foreground uppercase tracking-wider"><LayoutGrid className="mr-2 h-4 w-4"/> Homepage Sections</h3>
               <FormField
                control={form.control}
