...
export default function ArtistRegistrationToggleTab() {
  ...
  useEffect(() => {
    if (!isLoadingAppConfig && appConfig) {
      setIsRegistrationEnabled(appConfig.isArtistRegistrationEnabled === undefined ? true : appConfig.isArtistRegistrationEnabled);
    }
  ...
  const handleSaveChanges = async () => {
    ...
    try {
      ...
      const dataToSave: Partial<AppSettings> = {
        isArtistRegistrationEnabled: isRegistrationEnabled,
        updatedAt: Timestamp.now(),
      };
      await setDoc(settingsDocRef, dataToSave, { merge: true });
      toast({ title: "Success", description: "Artist registration access updated." });
    } catch (error) {
        ...
        <div className="flex flex-row items-center justify-between rounded-lg border p-4 shadow-sm">
          <div className="space-y-0.5">
            ...
            <p className="text-sm text-muted-foreground">
              {isRegistrationEnabled ? "Enabled: New Artists can register." : "Disabled: Registration page will show 'currently closed'."}
            </p>
          </div>
          <Switch
            ...
            disabled={isSaving}
            aria-label="Toggle Artist registration"
          />
        </div>
      </CardContent>
