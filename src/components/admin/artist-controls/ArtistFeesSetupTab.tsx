...
import type { AppSettings, ArtistFeeType } from '@/types/firestore';
import { useApplicationConfig } from '@/hooks/useApplicationConfig';

const ArtistFeesSchema = z.object({
  artistFeeType: z.enum(['fixed', 'percentage'], { required_error: "You must select a fee type."}),
  artistFeeValue: z.coerce.number().min(0, "Fee value must be non-negative."),
}).refine(data => {
    if (data.artistFeeType === 'percentage' && data.artistFeeValue > 100) {
        return false;
    }
    return true;
}, {
...
type ArtistFeesFormData = z.infer<typeof ArtistFeesSchema>;

export default function ArtistFeesSetupTab() {
  const { toast } = useToast();
  const { config: appConfig, isLoading: isLoadingAppConfig } = useApplicationConfig();
  const [isSaving, setIsSaving] = useState(false);

  const form = useForm<ArtistFeesFormData>({
    resolver: zodResolver(ArtistFeesSchema),
    defaultValues: {
      artistFeeType: 'fixed',
      artistFeeValue: 0,
    },
  ...
  useEffect(() => {
    if (!isLoadingAppConfig && appConfig) {
      form.reset({
        artistFeeType: appConfig.artistFeeType || 'fixed',
        artistFeeValue: appConfig.artistFeeValue || 0,
      });
    }
  }, [appConfig, isLoadingAppConfig, form]);

  const onSubmit = async (data: ArtistFeesFormData) => {
    setIsSaving(true);
    try {
      const settingsDocRef = doc(db, "webSettings", "applicationConfig");
      const settingsToUpdate: Partial<AppSettings> = {
        artistFeeType: data.artistFeeType,
        artistFeeValue: data.artistFeeValue,
        updatedAt: Timestamp.now(),
      };
      await setDoc(settingsDocRef, settingsToUpdate, { merge: true });
      toast({ title: "Success", description: "Artist fee settings have been saved." });
    } catch (error) {
      console.error("Error saving Artist fee settings:", error);
                ...
                <FormItem>
                  ...
                  <FormControl>
                    <Input
                      type="number"
                      step="0.01"
                      placeholder={form.getValues("artistFeeType") === 'percentage' ? "e.g., 20" : "e.g., 50"}
                      {...field}
                      disabled={isSaving}
                    />
                  </FormControl>
