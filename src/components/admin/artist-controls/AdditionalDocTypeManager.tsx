...
const COLLECTION_NAME = "ArtistControlOptions";
...
export default function AdditionalDocTypeManager() {
  ...
  const [isKycEnabled, setIsKycEnabled] = useState(true);
  ...
  const fetchOptions = useCallback(async () => {
    ...
    try {
      ...
      if (docSnap.exists()) {
        ...
        setIsKycEnabled(data.isKycEnabled ?? true);
      } else {
        setOptions([]);
  ...
  const handleToggleKyc = async (enabled: boolean) => {
    setIsSubmitting(true);
    try {
      const docRef = doc(db, COLLECTION_NAME, DOCUMENT_ID);
      await setDoc(docRef, { isKycEnabled: enabled, updatedAt: Timestamp.now() }, { merge: true });
      setIsKycEnabled(enabled);
      toast({ title: "Success", description: `KYC Step ${enabled ? 'enabled' : 'disabled'}.` });
    } catch (error) {
      toast({ title: "Error", description: "Could not update KYC setting.", variant: "destructive" });
    } finally {
  ...
  const onSubmit = async (data: AdditionalDocTypeFormData) => {
    ...
    const processedData = {
      ...data,
      docNumberMinLength: data.docNumberMinLength ?? null,
      docNumberMaxLength: data.docNumberMaxLength ?? null,
    };
    ...
    try {
      const docRef = doc(db, COLLECTION_NAME, DOCUMENT_ID);
      await setDoc(docRef, { 
        [ARRAY_FIELD_NAME]: updatedOptionsArray, 
        updatedAt: Timestamp.now() 
      }, { merge: true });
      ...
      toast({ title: "Error", description: `Could not save document type. Please try again.`, variant: "destructive" });
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
          <CardTitle className="text-xl flex items-center"><Paperclip className="mr-2 h-5 w-5"/>KYC & Additional Documents</CardTitle>
          <CardDescription>Configure document requirements for Artists during registration.</CardDescription>
        </div>
        <div className="flex items-center gap-6">
          <div className="flex items-center gap-2 border rounded-lg px-3 py-1.5 bg-muted/20">
            <div className="flex flex-col">
              <span className="text-xs font-bold leading-none">KYC Step</span>
              <span className={cn("text-[10px]", isKycEnabled ? "text-green-600" : "text-muted-foreground")}>
                {isKycEnabled ? "Enabled" : "Disabled"}
              </span>
            </div>
            <Switch 
