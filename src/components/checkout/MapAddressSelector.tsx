...
interface MapAddressSelectorProps {
  ...
  onMountLocate?: boolean; // Trigger geolocation on mount
}
...
const GOOGLE_MAPS_SCRIPT_ID = "newtalent-google-maps-places-script";
const GOOGLE_MAPS_CALLBACK_NAME = `initNewtalentMapAddressSelectorCallback_${Math.random().toString(36).substring(2, 15)}`;


const MapAddressSelector: React.FC<MapAddressSelectorProps> = ({ apiKey, onAddressSelect, onClose, initialCenter, onManualSearchClick, onMountLocate }) => {
  ...
  useEffect(() => {
    initMapAndControls();
    if (onMountLocate && isScriptLoaded) {
        handleLocateMe();
    }
  }, [initMapAndControls, onMountLocate, isScriptLoaded, handleLocateMe]);
