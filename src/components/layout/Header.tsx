...
const Header = () => {
  ...
  useEffect(() => {
    const getInitialTheme = () => {
        const storedTheme = localStorage.getItem('newtalent-theme') as 'light' | 'dark' | null;
    ...
    return () => {
        ...
        window.removeEventListener('storage', (event) => {
            if (event.key === 'newtalent-theme') handleThemeChange();
        });
    };
  }, []);
  ...
  useEffect(() => {
    ...
    const handleStorageChange = (event: StorageEvent) => {
      if (event.key === 'newtalentUserCart' || event.key === null) {
        updateCartCount();
      }
    };
  ...
  return (
    <>
        ...
        <div data-hydration-state="hydrated" className="container mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <div className="flex items-center gap-8">
            ...
            <nav className="hidden md:flex items-center gap-2">
              ...
              { !isLoadingAppConfig && (appConfig.isArtistRegistrationEnabled || (user && user.email === ADMIN_EMAIL)) && !firestoreUser?.roles?.includes('artist') && (
                <button
                  onClick={handleJoinAsArtist}
                  className={cn(
                    "px-4 py-2 text-sm font-semibold rounded-full transition-all duration-300 flex items-center",
                    currentPathnameFromHook === '/artist-registration'
                      ? "bg-primary text-primary-foreground shadow-md"
                      : "bg-muted/50 text-foreground/70 hover:bg-primary hover:text-primary-foreground hover:shadow-md"
                  )}
                >
                  <UserPlus className="mr-2 h-4 w-4" /> Join as Artist
                  </button>              )}
            </nav>
          </div>
      ...
      <SubscriptionPlansDialog 
        ...
        onSuccess={() => {
            toast({ title: "Success", description: "You can now join as an artist!" });
            router.push('/artist-registration');
        }}
      />
    </>
