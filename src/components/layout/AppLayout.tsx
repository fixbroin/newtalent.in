...
const AppLayout: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  useEffect(() => {
    if (isClientMounted) { 
      ...
      const shouldShowBottomNav = isMobile && (bottomNavActivePaths.includes(pathname) || isArtistProfile);
  ...
  const shouldShowMobileTabs = isClientMounted && isMobile && (pathname === '/' || pathname === '/artist-registration' || pathname === '/script-writing');
    ...
    <div className="flex flex-col min-h-screen">
      {shouldShowHeader && (
       <div data-site-header className={cn(
            ...
            <Header />
            {shouldShowMobileTabs && <MobileHomeTabs />}
        </div>
      )}
     <main
  className={cn(
    ...
    shouldShowHeader && (shouldShowMobileTabs ? "pt-[112px]" : "pt-[64px]")
  )}
>
