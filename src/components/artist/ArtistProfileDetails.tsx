...
import { Mail, User, Phone, ShieldCheck, ShieldAlert, Edit3, Loader2, AtSign } from "lucide-react";
...
export default function ArtistProfileDetails({
    user,
    firestoreUser,
    ...
    const handleSendVerificationEmail = async () => {
        ...
        if (!auth.currentUser || !emailToVerify) {
            toast({ title: "Email not found", description: "Artist email is not set in the profile.", variant: "destructive" });
            return;
        }
        ...
        if (auth.currentUser.email === emailToVerify && !auth.currentUser.emailVerified) {
             ...
             try {
                const actionCodeSettings = { url: `${window.location.origin}/artist/profile`, handleCodeInApp: true };
        ...
        <Card className="shadow-lg">
          ...
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <h3 className="text-lg font-semibold flex items-center"><AtSign className="mr-2 h-5 w-5 text-primary" /> Username</h3>
              <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-md">
                <p className="text-muted-foreground">@{firestoreUser?.username || "Not set"}</p>
                {allowUsernameEdit && onEditUsername && (
                    <Button variant="ghost" size="sm" onClick={onEditUsername}><Edit3 className="mr-1 h-4 w-4" /> Edit</Button>
                )}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between p-3 bg-secondary/30 rounded-md">
                ...
                {onEditName ? (
                 ...
                 <Link href="/artist-registration?edit=profile" passHref>
