...
interface NewJobArtistPopupProps {
  isOpen: boolean;
  bookingDocId: string;
...
export default function NewJobArtistPopup({
  isOpen,
  bookingDocId,
  bookingHumanId,
  onClose,
}: NewJobArtistPopupProps) {
  const router = useRouter();
  const { showLoading } = useLoading();

  const handleViewJob = () => {
    showLoading();
    // Path for Artist booking details
    router.push(`/artist/booking/${bookingDocId}`);
