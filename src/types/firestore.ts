...
export interface FirestoreCategory {
  ...
  isPortfolioRequired?: boolean; // Added
  imageUrl?: string;
  imageHint?: string;
...
export interface FirestoreBooking {
  ...
  bookingId: string; // User-friendly booking ID (e.g., Newtalent-TIMESTAMP-RANDOM)
  userId?: string; // If user is logged in
  artistId?: string; // ID of the assigned artist
  customerName: string;
...
export type ConnectionRequestStatus = 'pending' | 'accepted' | 'rejected' | 'closed';
...
// User type updated with marketing and subscription fields
export interface FirestoreUser {
  ...
  // New/updated for artist settlement
  withdrawableBalance?: number; // Total net earnings ready for withdrawal
  totalPaidOut?: number; // Lifetime artist payouts
  monthlyStats?: {
  ...
  showSocialMediaOnPublicProfile?: boolean;
  socialMediaLinks?: {
    facebook?: string;
    instagram?: string;
    twitter?: string;
...
export type ArtistFeeType = 'fixed' | 'percentage';
...
export interface AppSettings {
  ...
  siteName?: string;
  logoUrl?: string;
  isArtistRegistrationEnabled?: boolean; // For toggling registration
  maxArtistRadiusKm?: number; // New for artist work area
  ...
  isSubscriptionRequired?: boolean; // New: Toggle to require paid subscription for requests/artist role

  // Artist Fee Settings
  artistFeeType?: ArtistFeeType;
  artistFeeValue?: number;

  allowUsernameEdit?: boolean; // Toggle for username editing in profile
...
export type PopupDisplayRuleType =
  | "on_page_load"
  | "on_exit_intent" | "after_x_seconds"
...
export interface ChatSession {
  ...
  blockedBy?: string[]; // Array of UIDs who have blocked this chat
  createdAt: Timestamp;
  updatedAt: Timestamp;
  }
// Inquiry Types
...
export interface FirestoreQuotation {
  id?: string; // Firestore document ID
  artistId: string; // Added to scope to artist
...
// --- Artist Registration & Management Types ---
...
export interface OptionalDocumentTypeOption {
  ...
  docNumberMinLength?: number | null;
  docNumberMaxLength?: number | null;
  createdAt: Timestamp;
  updatedAt?: Timestamp;
}
...
export interface ArtistControlOptions {
  ...
  additionalDocTypes: AdditionalDocumentTypeOption[];
  isKycEnabled?: boolean; // Added
...
export type ArtistApplicationStatus = 'pending_step_1' | 'pending_step_2' | 'pending_step_3' | 'pending_step_4' | 'pending_step_5' | 'pending_review' | 'approved' | 'rejected' | 'needs_update';
...
export interface ArtistApplication {
  ...
  workCategorySlug?: string; // Added for SEO URLs
  experienceLevelId?: string;
  experienceLevelLabel?: string; // Denormalized
  gender?: string;
  bio?: string; // Added field for short bio
  username?: string; // Added for public profile URL
  ...
  // Artist Earnings/Wallet - these should perhaps be on the main User document for a artist role
  withdrawableBalance?: number;
  totalEarnings?: number;
  totalDeductions?: number;
  promotionIndex?: number; // Optional rank for display priority (1, 2, 3...)
}



// This would be the main profile for an approved artist
export interface ArtistProfile extends Omit<ArtistApplication, 'status' | 'submittedAt' | 'adminReviewNotes'> {
