'use server';
/**
 * @fileOverview A Genkit flow to send a subscription expiry email to a user.
 */
...
// Input schema for the subscription expiry email flow
const SubscriptionExpiryEmailInputSchema = z.object({
...
export type SubscriptionExpiryEmailInput = z.infer<typeof SubscriptionExpiryEmailInputSchema>;

export async function sendSubscriptionExpiryEmail(input: SubscriptionExpiryEmailInput): Promise<{ success: boolean; message: string }> {
  try {
    return await subscriptionExpiryEmailFlow(input);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return { success: false, message: `Failed to process subscription expiry email flow: ${errorMessage}` };
  }
}
...
const subscriptionExpiryEmailFlow = ai.defineFlow(
  {
    name: 'subscriptionExpiryEmailFlow',
    inputSchema: SubscriptionExpiryEmailInputSchema,
    outputSchema: z.object({ success: z.boolean(), message: z.string() }),
  },
  async (details) => {
    const { smtpHost, smtpPort, smtpUser, smtpPass, senderEmail, userName, userEmail, siteName = "Newtalent", logoUrl } = details;
    
    const canAttemptRealEmail = smtpHost && smtpPort && smtpUser && smtpPass && senderEmail;

    const subscriptionsUrl = `${getBaseUrl()}/subscriptions`;
    const emailSubject = `Important: Your Subscription at ${siteName} has Expired`;
    const emailBodyContent = `
        <p>Hi ${userName},</p>
        <p>Your premium subscription at ${siteName} has expired.</p>
        <p>To continue enjoying unlimited connection requests, artist profile visibility, and other premium features, please renew your subscription.</p>
        <p><a href="${subscriptionsUrl}" class="button">Renew Subscription</a></p>
        <p>If you have any questions or need assistance with renewal, our support team is here to help.</p>
        <p>Best regards,<br>The ${siteName} Team</p>
    `;
    const htmlBody = createHtmlTemplate(`Your Subscription has Expired`, emailBodyContent, siteName, logoUrl);

    if (!canAttemptRealEmail) {
      console.warn("SMTP configuration incomplete. Simulating expiry email.");
