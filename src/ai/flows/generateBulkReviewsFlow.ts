...
'use server';
/**
 * @fileOverview An AI flow to generate a batch of realistic reviews for an artist or category.
 */
...
const GenerateBulkReviewsInputSchema = z.object({
  serviceId: z.string().describe("The ID of the artist or category (prefixed with category_) to generate reviews for."),
  serviceName: z.string().describe("The name of the artist or category to generate reviews for."),
  categoryName: z.string().describe("The category context for the artist or the category itself."),
  subCategoryName: z.string().optional().describe("Not used for artists, but kept for schema compatibility."),
...
const generateReviewsPrompt = ai.definePrompt({
    ...
    prompt: `You are an expert content generator for an artist discovery platform called "Newtalent".
Your task is to generate a batch of realistic user reviews for a specific artist or talent category.
The reviews should sound authentic, use common Indian names, and have a mix of positive tones.

Artist/Category Name: {{serviceName}}
Primary Category: {{categoryName}}

{{#if existingNames}}
The following names are already used for existing reviews of this profile. DO NOT use any of these names:
{{#each existingNames}}
- {{this}}
{{/each}}
{{/if}}
...
3.  **comment**: A short, natural-sounding review comment between 10 and 80 words. The tone should vary (e.g., "Amazing talent!", "Very professional and easy to work with.", "Great portfolio, looking forward to connecting again.").
    If this is for a category, reviews should be about the general quality of talent found in that category. 
    If this is for a specific artist, reviews should be about that artist's skills and professionalism.
