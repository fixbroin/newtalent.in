 ...
 *
 * Enhanced production-ready AI chat flow for Newtalent.
...
function buildSystemPrompt(params: {
  ...
  return `
You are the official Newtalent AI Support Specialist. Your goal is to provide accurate, helpful, and concise information about Newtalent's services, locations, and policies.
  ...
  async (input) => {
    ...
    const triggerSupportEmail = async (msg: string) => {
        ...
        await sendHumanSupportRequestEmail({
            ...
            siteName: "Newtalent Support Alert",
        });
    };

    // 1) Greeting
    if (isGreeting(message)) {
      return { response: `Hi ${name}! I'm your Newtalent assistant. How can I help you with our services or your bookings today?` };
    }
    ...
    if (isLocationIntent(message)) {
        ...
        if (msg.includes('where') || msg.includes('city') || msg.includes('area')) {
            ...
            return { response: `Newtalent currently operates in ${cityNames}. We cover many areas including ${locations.areas.slice(0, 5).map(a => a.name).join(', ')}, and more!` };
        }
    }
