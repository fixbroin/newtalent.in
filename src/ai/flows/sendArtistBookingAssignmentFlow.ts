 ...
 *
 * - sendArtistBookingAssignmentEmail - Sends an email to the Artist.
 * - ArtistBookingAssignmentEmailInput - The input type for the flow.
 */
...
const ArtistBookingAssignmentEmailInputSchema = z.object({
...
export type ArtistBookingAssignmentEmailInput = z.infer<typeof ArtistBookingAssignmentEmailInputSchema>;

export async function sendArtistBookingAssignmentEmail(input: ArtistBookingAssignmentEmailInput): Promise<{ success: boolean; message: string }> {
  try {
    return await ArtistBookingAssignmentEmailFlow(input);
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    return { success: false, message: `Failed to process Artist assignment email flow: ${errorMessage}` };
  }
}
...
const ArtistBookingAssignmentEmailFlow = ai.defineFlow(
  {
    name: 'ArtistBookingAssignmentEmailFlow',
    inputSchema: ArtistBookingAssignmentEmailInputSchema,
  ...
  async (details) => {
    try {
      const {
        ...
        ArtistName, ArtistEmail, bookingId, bookingDocId, serviceName,
      ...
      const jobDetailsUrl = `${appBaseUrl}/artist/booking/${bookingDocId}`;
      ...
      if (!canAttemptRealEmail) {
        console.warn("SMTP configuration incomplete. Simulating Artist assignment email.");
      ...
      await transporter.sendMail({
        from: `${siteName} Operations <${senderEmail}>`, to: ArtistEmail, subject: emailSubject, html: htmlBody,
      });
      
      return { success: true, message: "Artist assignment email sent successfully." };

    } catch (error: any) {
      console.error("Error in ArtistBookingAssignmentEmailFlow:", error);
